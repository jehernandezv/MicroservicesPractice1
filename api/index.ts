...
class Server {
    ...
    routes(turn:Number): void {
        this.app.get('/:min/:max', async function (req, res) {
            ...
            axios.get('http://'+ `${(turn == 0)?process.env.SERVER_UBUNTU1:process.env.SERVER_UBUNTU2}`+':' + process.env.PORT_SERVER_UBUNTU + '/api/ubuntu1/' + min + '/' + max).then(async function (response) {    
